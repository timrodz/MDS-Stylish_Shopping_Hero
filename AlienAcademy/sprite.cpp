...
bool CSprite::Initialise(std::wstring _strFileName, std::wstring _strFileMask, int _iWidth, int _iHeight)
{
	...
	m_hSprite = (HBITMAP)LoadImageW(NULL, _strFileName.c_str(), IMAGE_BITMAP, _iWidth, _iHeight, LR_LOADFROMFILE);
	VALIDATE(m_hSprite);
	m_hMask = (HBITMAP)LoadImageW(NULL, _strFileMask.c_str(), IMAGE_BITMAP, _iWidth, _iHeight, LR_LOADFROMFILE);
	VALIDATE(m_hMask);
